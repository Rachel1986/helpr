...
public interface FuncionarioRepository extends JpaRepository<Funcionario, Integer> { //Generics (<>)
    List<Funcionario> findByCargo(Cargo cargo); // Filtrar os funcionarios que possuem cargo
    List<Funcionario> findByFotoIsNull(); // Filtrar funcionários sem foto definida
